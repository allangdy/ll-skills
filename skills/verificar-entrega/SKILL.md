...
Um implementador autônomo declarou pronto. Esta skill decide se está. Ela audita, nunca conserta: falha encontrada vira achado com evidência e, quando é estrutural, vira decisão nova para `decidir-antes` — nunca patch silencioso desta sessão.
...
Feche com a lista de pendências que realimenta `decidir-antes`: cada falha estrutural — a que exige escolher de novo, não corrigir — vira uma decisão a ser tomada, com as opções e o custo de cada uma. Falha local (teste quebrado, bug pontual) fica como item de correção, sem virar decisão.

Apresente ao usuário, em poucas linhas: o veredicto, a contagem por severidade, os BLOQUEIA ENTREGA nomeados, e as decisões que sobraram para ele — com a sugestão de rodar `decidir-antes` sobre elas se houver mais de uma estrutural. Você entrega o relatório e para aqui.
...
Rode as camadas (a) e (b) contra esses critérios; a camada (c) cai — sem spec versionada não há integridade de processo a auditar. Feche sugerindo `decidir-antes` para a próxima entrega, para que a verificação seguinte tenha um oráculo em vez de uma reconstrução.
