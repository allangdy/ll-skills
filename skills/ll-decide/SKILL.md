...
### 4. Decision room
Write `docs/decide/OPTIONS.html` — self-contained: one alternatives table per topic (design, architecture, flow, data model) with what becomes true, cost, what is lost, revert cost and the recommendation, plus the fixed block with the five gate answers. A Fable judge in clean context (never fork) reads the file and the evidence and returns an opinion per topic; the opinion is appended to the file as its own section. The file is the deliverable; its path is sent to the owner in a message of its own, before the first interview question — no other tool publishes it and no other skill is loaded to write it. `references/decision-room.md`.
