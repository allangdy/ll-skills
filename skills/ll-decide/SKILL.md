...
| none of these returns the content | owner-only question: ask him to attach the file or paste the content — never an assumption about what it contains |
...
Four questions in one block, before any subagent: the number that decides success and what counts as FAILURE; the deliverable in the client's format; the source of truth for the data; the house pattern that applies. One tool call carries the four — never a fifth question in a second call. The fifth premise — a constraint the session is inventing out of caution that is not the owner's rule — is never asked: it is dropped, or kept as an assumption `ASM-n [revisable]` with its review trigger, and named as an assumption in the count line. Skip every question `OPENING.md` already answers. Option shapes and why each question exists: `references/premise-gate.md`.

The gate is passed when each of the four premises carries a source — `file:line`, the owner's message quoted, or an `ASM-n` label he has seen. A premise without a source is one question block (≤4) sent now, and nothing after this step runs — no premortem, no narrator, no subagent — until it is answered or ratified. A blanket delegation does not open this gate; it only turns band-2/3 premises into `ASM-n`. Under `--no-talk` no block is sent: a premise without a source takes the recommended answer of its question as `ASM-n [decided by absence — revisable]`, and the first two — owner-only, like any premise resting on a reference this session could not read — are written as premises and also as `decisions/DEC-NNNN-<slug>.md` in state `WAITING`, so the later steps run.
...
Minimum contact: at least one message reaches the owner before PLAN.md is frozen, whatever the delegation was. It carries, in this order, (a) the decision count, (b) the counter `questions asked N / assumptions M / owner decisions open K` (printed to the owner in Portuguese as `perguntas N / assunções M · decisões só suas em aberto K`), (c) the owner-only items still waiting for an answer, then every decision as it will be written — ID, decision, who decided, against-recommendation marks, assumptions. A run whose items are all band 2/3 still sends it, with `N = 0` and the ASM list. An item that was badly posed is re-asked here; a correction is applied and the list shown once more. "ok" or "A" freezes; ten minutes of silence ratifies the recommended list (A, the assumptions) and freezes PLAN.md too, but only when `owner decisions open` is 0 — with one owner-only item open nothing freezes. Under `--no-talk` the round is printed as a report, not asked: same content, counter `questions asked 0 / assumptions M / owner decisions open K`, and the contract is written and frozen even with K > 0 — the `WAITING` DECs are what stop the dependent work, not this skill. Nothing is written before this round closes.
...
### 8. Hand off
Report in Portuguese, opening with the counter `questions asked N / assumptions M / owner decisions open K` (printed as `perguntas N / assunções M · decisões só suas em aberto K`), then the decision counts (owner / assumption / inherited / against recommendation), accepted risks, files written, hand-check result. Under `--no-talk` the report also names each `WAITING` DEC with its file path and the phase that carries `stop: owner`. With `N = 0` and `M > 5`, append one line under the phase heading of `PROGRESS.md`: `- review assumptions: <M> ASM written with 0 questions asked (<date>)`.
...
- Blanket delegation ("decide tudo"): covers bands 2 and 3; owner-only items are asked anyway, and an owner reference this session could not read is band 1.
- Final round: always sent before writing, with the counter and the open owner-only list, even when zero questions were asked.
- `--no-talk`: steps 1, 5 and 6 send no question; band-2/3 items take the recommendation, recorded `[decided by absence — revisable]`; owner-only items are written as `WAITING` decisions and the contract is frozen anyway.
- Never asked: the eleven items of `references/decision-policy.md` (who executes, a fact readable from the repo, which pattern when a house pattern exists, a reversible detail inside a closed contract, another session's decision, out-of-scope subjects, a question that changes no action, copy without a commercial promise, industry defaults, the same policy question twice, options that differ only in rigor). Silence for 10 minutes ratifies the recommended option of a band-2/3 item, never option B and never band 1.
...
- Step 1 — the four questions, option shapes, where each answer lands: [references/premise-gate.md](references/premise-gate.md)
