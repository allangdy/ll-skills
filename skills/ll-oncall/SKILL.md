...
1. **Contract.** Read the project `CLAUDE.md`. No `## Federation` block → write it (template P1 in
   ...
   executes, the path denylist, the shared resources it owns. The boundary holds when it is a written
   negative list — *"quem faz CI e CD é o agente da infra e não você"* ended the incursions into
   `.github/` the day it was written down.
2. **Peer map.** `ListAgents`, resolve each peer prefix to the current `name [id]`, keep it for this turn
   ...
   not: one session answered to six names in four days, and all 51 addressing failures were in suffixes.
...
3. **Trigger with two conditions**, written before the first cycle, joined by AND: the anomaly outside
   the baseline *and* a second signal pointing at the suspect. One watch ran three days and 50 cycles
   with zero false positives on that rule.
...
6. **Degradation.** Three cycles in the same blocked state → change channel, tell the owner directly,
   stop repeating the line; 35 identical hourly messages produced 35 blind hours. Five green cycles →
   double the interval.
...
2. **The command that needs the owner's hand goes out in the first turn**, ready to paste, with what it
   does and costs — not at the end. The same `apply` was blocked four times in eleven days.
3. **Pre-deploy that exercises the exact operation**, layer by layer: credential → container → TLS →
   privilege → schema. Six deploy rounds once found four causes one at a time, all checkable up front.
...
Never ask: *"confirma o que a outra sessão disse?"* — eight of those in a row were all confirmed; who
executes; how many worktrees; which model; a fact readable from the repo, the cluster or the database;
