...
1. **Reconcile.** `ll-tools.js backlog-reconcile --run`. Items whose condition passed close; the rest stay open with their exit code. Then add the items born in this delivery — deviations, stubs, tests not run, debt, domain questions — one row each with the next free `B-nnn`, an executable closing condition (`` `<command>` exit 0 ``), state `OPEN`, and its birth phase and commit. An item without a condition is a wish, not a backlog entry.
