#!/usr/bin/env node
'use strict';
// ll-tools.js — ll-skills helper. Node >= 18, no deps. Reads always exit 0
// ({"ok":false,"reason":...} on failure); writes exit 1 on error.
...
function findDown(base, name, maxDepth) { // dirs containing `name`, depth <= maxDepth, skipping junk
  const out = [];
...
function gitTop(root) { return git(['rev-parse', '--show-toplevel'], root).trim() || root; } // root may be under git top
...
function commitRe(id) { // anchored: (M1) never matches feat(M10)
  const e = String(id).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return { full: new RegExp('^(test|feat|refactor)\\(' + e + '\\): '), loose: new RegExp('\\(' + e + '\\)') };
}
function kahn(dep) { // topological layers
...
C.waves = (a) => {
  ...
  const isRoot = (id) => { // ANSWERED_NO/BLOCKED, or `stop: owner` unaccepted
    const e = st[id];
...
C['tdd-gate'] = (a) => {
  ...
  const s = (c) => (c ? { sha7: c.sha7, type: c.type } : null); // never echo the subject
...
C['dec-reserve'] = (a) => {
  ...
  if (!pre) {
    // detect the dominant prefix in use: most frequent, ties -> shortest, none -> DEC
...
function stateLine(indent, id, f) { // fixed field order
...
C['backlog-reconcile'] = (a) => { // condition: `cmd` exit N | `cmd` = <stdout>
...
C.epilogue = (a) => {
  ...
  try { if (fs.existsSync(plan)) blocked_by = C.waves({ _: [plan], flags: { cwd: root } }).blocked_by; } catch { /* see `waves` */ }
  try {
...
C['phase-stats'] = (a) => {
  const es = gitLog(rootOf(a), a.flags.since ? ['--since=' + a.flags.since] : null);
...
const RULES = [ // rule, severity, predicate
